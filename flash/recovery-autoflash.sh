...
# --- token ---
if [ -z "${GH_TOKEN:-}" ]; then
  for f in /mnt/autoflash/gh_token /media/usb/gh_token /root/gh_token ./gh_token; do
    [ -r "$f" ] && { GH_TOKEN=$(tr -d ' \t\r\n' < "$f"); log "read token from $f"; break; }
  done
fi
[ -n "${GH_TOKEN:-}" ] || die "no GitHub token. export GH_TOKEN or put it in gh_token on the USB (needs Actions: read)."

api() { curl -fsSL -H "Authorization: Bearer $GH_TOKEN" -H "Accept: application/vnd.github+json" \
             -H "X-GitHub-Api-Version: 2022-11-28" "$@"; }

# --- storage: mount a USB partition for the ~200 MB download, else fall back to /tmp ---
MOUNTED=
if [ -z "${WORKDIR:-}" ]; then
  WORKDIR=/tmp
  for p in /dev/sda1 /dev/sda2 /dev/sdb1 /dev/sdb2; do
    [ -b "$p" ] || continue
    case "$(blkid -o value -s TYPE "$p" 2>/dev/null || true)" in
      ext4|ext3|ext2|vfat)
        mkdir -p /mnt/autoflash
        if mount "$p" /mnt/autoflash 2>/dev/null; then
          WORKDIR=/mnt/autoflash; MOUNTED=$p; log "using USB $p at $WORKDIR"; break
        fi ;;
    esac
  done
  [ -z "$MOUNTED" ] && log "no USB partition mounted; using $WORKDIR (RAM -- may be too small)"
fi
mkdir -p "$WORKDIR"
cleanup() { [ -n "$MOUNTED" ] && umount /mnt/autoflash 2>/dev/null || true; }
