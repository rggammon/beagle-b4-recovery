...
| `dc_nohw_kms_present.c`    | The Stage 3 presenter (hard-float, raw DRM ioctls, no libdrm): becomes DRM master and scans a framebuffer out on `DVI-D-1`. `dc_nohw_kms_present dumb [s]` = CPU colour bars in a dumb buffer; `dc_nohw_kms_present import [s]` = `PRIME_FD_TO_HANDLE` import of a `dc_nohw` DMA-BUF + `ADDFB2` + `SETCRTC`. Cross-build `arm-linux-gnueabihf`. |
